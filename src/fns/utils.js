...
/**
 * Searches through an array of lines to find the index of the first line that contains
 * the given tag after the given index.
 * 
 * @param {string[]} lines The array of lines to search through.
 * @param {string} tag The tag to search for in each line.
 * @param {number} [fromIndex=0] The index to start searching from.
 * @returns {number} The first matching index, or `-1` if no match.
 */
function getIndexOfNextLineMatch(lines, tag, fromIndex = 0) {
  return lines.findIndex((v, i) => i < fromIndex ? false : v.includes(`@${tag}`));
}
...
module.exports.getIndexOfNextLineMatch = getIndexOfNextLineMatch;
